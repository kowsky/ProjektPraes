...
Nun wird für jeden Suffix $suffix_{qx}$ im $Suffixarray_Q$ die Position $i$ im $Suffixarray_D$ ermittelt, an der der Suffix gemäß der lexikographischen Ordnung einzusortieren wäre. 
Ein Fenster der Größe $2h$ wird so über diese Position gelegt, dass es die $h$ Positionen vor und die $h$ Positionen nach dem hypothetisch einsortierten $suffix_{qx}$ überdeckt. 
Anhand dieses Fensters werden die SANS-Scores aktualisiert: 
Jeder Suffix $suffix_{dy}$ eines Proteins $y$ im Fenster erhöht den SANS-Score-Zähler der dazugezugehörigen Proteinkombination $(query_x,database_y)$ um eins. \\
>>>>>>> GSA quelle, text kleinigkeiten
